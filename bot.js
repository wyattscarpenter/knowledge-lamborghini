...
client.on('clientReady', () => {
  console.log("I am ready! I am kl (", version_string, ") logged in as", client.user?.tag, "with optional command prefix", command_prefix);
...
function send_long(channel, string, resume_quotatively=false, message_id_to_reply_to=null){
  //COULD someday make this function actually return an success bool (which could be checked before a backing record is updated) instead of just logging and giving up. OTOH, the underlying send already retries a couple of times (due to TCP) so what am I even protecting about? Times when the bot is up and wants to fire off a remindme but my server-side internet connection is down? Hmm actually that is a fairly good thing to defend against. The power on the router could go out at any moment... and it would be better if we didn't just drop it on the floor... we also probably want to handle some specific cases like "you've been kicked from the server, actually", so we don't just infinitely retry on those. Or maybe we can infinitely retry on specifically a network outage code? Maybe the success bool is ultimatley only important for the remindmes, which have a unique re-dispatching architecture anyway. TODO. Then, update the readme to once again claim that "For everything except remindmes and starboards (features where this would be extremely annoying), this bot has a sort of loosey-goosey attitude" instead of just starboards (which only have the better error handling still in them because it forwards a message, which neither requires send_long nor can be done with send_long, so I left that error handling in place — and even then, it just prevents the backing record from being updated, so that you can retrigger the starboard trigger by reacting again, without it being ruled out by the list of things that have already been reacted. So, maybe not anything a user would be thinking of as an exception to loosey-gooseyness)
