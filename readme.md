...
* `keep a starboard here` (starting a message; followed by any or no other text) will cause kl to repost a messages in that channel whenever it is running an a emoji gets five (5) reactions of the same type (note: unlike other starboards, not just the star emoji, but any reaction). `don't keep a starboard here` will turn this feature off for a channel it is posted in. Please note that this feature is very bare-bones and not 100% ideally-implemented because I got sick of working on it. But it does work, in its basic functionality.
