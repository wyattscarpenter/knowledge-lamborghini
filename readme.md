...
* NO special sigil, all commands are invoked as they are written here.
  * However, you are allowed to use a prefix `!` or `! ` on any command, which will be largely ignored, since I know some people are in that habit. This prefix can also be set with the first argument to kl on the command line; this is largely for testing purposes.
...
* Type `set-for-channel (number) [keyword] [response message]` and kl will respond with the response message when the keyword is posted in the channel, as the only text in a message (case-insensitive). This is a per-channel setting!
  ...
  * (TODO) To avoid an infelicity in the way discord handles links, links of the form `https://cdn.discordapp.com/attachments/522287497697766714/689602347856091837/image_named_whatever.png?ex=04813579&is=1e3b4f9a&hm=2c3e455146676c93d186698f82b14c744e3df25d24286dd620301cb6f00479ab&` will have the discord timing parameters stripped off when they appear in response messages (including attachments), eg they will look like: `https://cdn.discordapp.com/attachments/522287497697766714/689602347856091837/image_named_whatever.png`. This is to prevent the unfortunate scenario where you try to `unset` a link but discord is automatically refreshing the timing parameters so the link stored inside the bot is not the same as the link you get from discord. (That state of affairs will still occur, but the bot will now ignore the difference.) Other types of parameters like width and height are not stripped (although, they don't seem to me to do anything, in my brief experimentation?). If a url like this has no parameters, it will not have a ? (the ?, and any extra ampersands, will be stripped if provided). The following is also normalized in discord attachment urls in response messages: http will be converted to https, ~~and the domain media.discordapp.net will be converted to cdn.discordapp.com~~ (actually, media.discordapp.net seems to be semantically different from cdn.discordapp.com [in some subtle way](https://old.reddit.com/r/discordapp/comments/9z88ci/cdndiscordappcom_and_mediadiscordappnet_why_both/), so they are not interconverted). Normalization is applied both on input and output of response messages.
  * (TODO) If the only thing in a response message is a link to cdn.discordapp.com or media.discordapp.net, send it as an attachment instead — this allows audio links to be properly previewed in a playable way.
