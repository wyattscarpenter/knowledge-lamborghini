...
* NO uptime, or stability guarantees. I commit broken features directly to master as I work on them. I may or may not have the bot running at any given time.
  * However, this bot is also published as [a package](https://www.npmjs.com/package/knowledge-lamborghini) on npmjs.org, and only non-broken versions should (in the optative sense) make it up there.
...
* This bot also is not ironclad against potential "supply chain" security attacks based on its transitive dependencies. I haven't read those dependencies' sources and I'm not going to. I run `npm audit fix` occasionally, and if you're hosting this bot yourself you should also probably do this. (Inviting this bot to your discord server, or interacting with it, is not "hosting" it in this sense.) Keep in mind, however, that npm audit often lists unimportant and nonthreatening "vulnerabilities", even as "high" priority; as detailed in e.g. https://overreacted.io/npm-audit-broken-by-design/. Also keep in mind that `npm audit fix --force` seems to usually downgrade the discordjs dependency and thus is basically useless. If you're developing this bot and want to update the dependencies, you should consider using npm's `min-release-age` feature.
...
* For everything except starboards this bot has a sort of loosey-goosey attitude towards correctness, and usually doesn't do anything in particular to catch errors except log them in a log file; so, if there are transitory networking issues between the bot and discord, a couple of messages might get dropped here and there. Feel free to bring these incidents up as issues on GitHub or batphone if they annoy you, and/or fix my code. Note: it's not always obvious what the most desirable error-handling behavior even should be.
* The bot occasionally experiences planned or unplanned "downtime", during which it cannot receive or send any messages. It will usually appear offline to discord during these times. Sometimes these outages are expected in advance, but I don't announce them anywhere.

### Positive features

A note about the commands: all commands are usually just as they are written here. Most commands have to be in their own message, and can't be put into the middle, beginning, or end, of another message. Parameters to commands (text used as data by the command) are noted below with descriptive phrases in square brackets, [like so]; replace the entire bracketed word/phrase, including the brackets, with the desired parameter, when invoking the command. Optional parameters are in parentheses. Parameters are typically not allowed to include whitespace, but sometimes the last parameter to a command can. The order of parameters is significant, as that's what allows kl is figure out which parameter is intended to mean what.
